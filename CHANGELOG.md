...
- `THIRD-PARTY-NOTICES.md` added at the repo root (shipped runtime dependencies with version and licence) and packed into the NuGet package. (#298) (#389)

### Internal

- Built against Wolfgang.Etl.Abstractions / ErrorPolicies / TestKit / TestKit.Xunit 0.26.0 (trim- and native-AOT-compatible on net8.0+; no API change from 0.25.0), so the package now requires Abstractions 0.26.0 or later. (#417)
- New `src/.editorconfig`: S1133 (scheduled `[Obsolete]` setters, #325) and S3427 (optional-parameter overlaps with the single-argument constructors that #295 retires) are off for the src project until the 2026-12-15 removal wave, with dated notes. (#404)
- Src clean-up: `RejectSerializerOptions` declares the null it already tolerates (`Options?`), the three loaders' `IsDryRun` get a real summary (not inherited), and the two extractors' guard-only `ApplyOptions` become static. (#405)
- Redundant `using`s removed from five options records and three loader test files; `ConstructorOverloadResolutionTests` marks its explicit trailing `null` arguments as deliberate. (#406)
- The options constructor assigns the stage's backing fields directly instead of going through the deprecated setters, so the `CS0618` suppressions that covered those writes are gone. None of the four stages' deprecated setters validate, so no record-side guard was needed. (#384) (#389)
- Record the compiler-synthesized members of the shipped records in `PublicAPI.Shipped.txt` (per-TFM files for the covariant `<Clone>$` lines); they were public all along, no surface change. (#329)
